...
public class AssetsDatabaseManager {
    ...
    public SQLiteDatabase getDatabase(String dbfile) {
        if(databases.get(dbfile) != null){
            LogUtils.i(tag, String.format("Return a database copy of %s", dbfile));
            return (SQLiteDatabase) databases.get(dbfile);
        }
        if(context==null)
            return null;

        LogUtils.i(tag, String.format("Create database %s", dbfile));
        ...
        if(!flag || !file.exists()){
            ...
            if(!copyAssetsToFilesystem(dbfile, sfile)){
                LogUtils.i(tag, String.format("Copy %s to %s fail!", dbfile, sfile));
                return null;
            }
